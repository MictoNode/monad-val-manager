//! Withdraw Dialog Widget rendering
//!
//! This module provides [`WithdrawDialogWidget`] for rendering the 2-field
//! Withdraw modal dialog in the TUI.
...
use crate::tui::staking::withdraw_state::{WithdrawField, WithdrawState};
use crate::tui::theme::THEME;

/// Widget for rendering the Withdraw dialog
pub struct WithdrawDialogWidget<'a> {
    /// Reference to dialog state
    state: &'a WithdrawState,
    /// Dialog width as percentage (0-100)
    width_percent: u16,
}

impl<'a> WithdrawDialogWidget<'a> {
    /// Create a new Withdraw dialog widget
    pub fn new(state: &'a WithdrawState) -> Self {
        Self {
            state,
            width_percent: 60,
        }
    ...
    fn calculate_dialog_area(&self, frame_area: Rect) -> Rect {
        ...
        let dialog_height = 14; // Fixed height for 2 fields + context hint + header + footer
    ...
    pub fn render(&self, frame: &mut Frame) {
        ...
        let chunks = Layout::default()
            ...
            .constraints([
                ...
                Constraint::Length(3), // Withdrawal ID field
        ...
        let instruction = Paragraph::new(Line::from(vec![Span::styled(
            "Enter Validator ID and Withdrawal ID to withdraw MON",
            THEME.metric_label(),
        )]))
        ...
        // Render each field
        self.render_field(frame, chunks[1], WithdrawField::ValidatorId);
        self.render_field(frame, chunks[2], WithdrawField::WithdrawalId);

        // Render context hint if available
        if let Some(ref hint) = &self.state.context_hint {
            let hint_line = Line::styled(format!(" {}", hint), THEME.status_info());
    ...
    fn render_field(&self, frame: &mut Frame, area: Rect, field: WithdrawField) {
        ...
        let textarea = match field {
            WithdrawField::ValidatorId => &self.state.validator_id,
            WithdrawField::WithdrawalId => &self.state.withdrawal_id,
        };
...
mod tests {
    ...
    #[test]
    fn test_withdraw_dialog_widget_creation() {
        let state = WithdrawState::new();
        let widget = WithdrawDialogWidget::new(&state);
        assert_eq!(widget.width_percent, 60);
    }

    #[test]
    fn test_withdraw_dialog_widget_with_width() {
        let state = WithdrawState::new();
        let widget = WithdrawDialogWidget::new(&state).with_width(80);
        assert_eq!(widget.width_percent, 80);
    }

    #[test]
    fn test_withdraw_dialog_widget_calculate_dialog_area() {
        let state = WithdrawState::new();
        let widget = WithdrawDialogWidget::new(&state).with_width(60);
        ...
        assert_eq!(dialog_area.height, 14); // Updated for context hint
    }
}
