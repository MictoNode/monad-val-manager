//! Change Commission Dialog Widget rendering
//!
//! This module provides [`ChangeCommissionDialogWidget`] for rendering the two-field
//! Change Commission modal dialog in the TUI.
...
use crate::tui::staking::change_commission_state::{ChangeCommissionField, ChangeCommissionState};
use crate::tui::theme::THEME;

/// Widget for rendering the Change Commission dialog
pub struct ChangeCommissionDialogWidget<'a> {
    /// Reference to dialog state
    state: &'a ChangeCommissionState,
    /// Dialog width as percentage (0-100)
    width_percent: u16,
}

impl<'a> ChangeCommissionDialogWidget<'a> {
    /// Create a new Change Commission dialog widget
    pub fn new(state: &'a ChangeCommissionState) -> Self {
        Self {
            state,
            width_percent: 60,
        }
    ...
    pub fn render(&self, frame: &mut Frame) {
        ...
        let chunks = Layout::default()
            ...
            .constraints([
                ...
                Constraint::Length(3), // Commission field
                Constraint::Length(1), // Current commission display
        ...
        let instruction = Paragraph::new(Line::from(vec![Span::styled(
            "Enter Validator ID and new Commission rate (0.0 - 100.0%)",
            THEME.metric_label(),
        )]))
        ...
        // Render each field
        self.render_field(frame, chunks[1], ChangeCommissionField::ValidatorId);
        self.render_field(frame, chunks[2], ChangeCommissionField::Commission);

        // Render current commission if available
        self.render_current_commission(frame, chunks[3]);
    ...
    fn render_field(&self, frame: &mut Frame, area: Rect, field: ChangeCommissionField) {
    ...
    /// Render current commission display
    fn render_current_commission(&self, frame: &mut Frame, area: Rect) {
        let content = if let Some(current) = self.state.current_commission {
            // current is already a percentage (e.g., 10.0 for 10%), no division needed
            Line::styled(format!(" Current: {:.1}%", current), THEME.status_info())
        } else {
            Line::styled(" Current: (enter validator ID first)", THEME.placeholder())
        };
...
mod tests {
    ...
    #[test]
    fn test_change_commission_dialog_widget_creation() {
        let state = ChangeCommissionState::new();
        let widget = ChangeCommissionDialogWidget::new(&state);
        assert_eq!(widget.width_percent, 60);
    }

    #[test]
    fn test_change_commission_dialog_widget_with_width() {
        let state = ChangeCommissionState::new();
        let widget = ChangeCommissionDialogWidget::new(&state).with_width(80);
        assert_eq!(widget.width_percent, 80);
    }

    #[test]
    fn test_change_commission_dialog_widget_width_capped_at_100() {
        let state = ChangeCommissionState::new();
        let widget = ChangeCommissionDialogWidget::new(&state).with_width(150);
        assert_eq!(widget.width_percent, 100);
    }

    #[test]
    fn test_change_commission_dialog_widget_calculate_dialog_area() {
        let state = ChangeCommissionState::new();
        let widget = ChangeCommissionDialogWidget::new(&state).with_width(60);
    ...
    #[test]
    fn test_change_commission_dialog_widget_minimum_width() {
        let state = ChangeCommissionState::new();
        let widget = ChangeCommissionDialogWidget::new(&state).with_width(30);
    ...
    #[test]
    fn test_change_commission_dialog_widget_builder_chain() {
        let state = ChangeCommissionState::new();
        let widget = ChangeCommissionDialogWidget::new(&state).with_width(70);

        assert_eq!(widget.width_percent, 70);
    }

    #[test]
    fn test_change_commission_dialog_widget_not_active() {
        let state = ChangeCommissionState::new();
