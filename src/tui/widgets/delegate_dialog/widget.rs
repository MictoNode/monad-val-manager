//! Delegate Dialog Widget rendering
//!
//! This module provides [`DelegateDialogWidget`] for rendering the 2-field
//! Delegate modal dialog in the TUI.
//!
...
use crate::tui::staking::delegate_state::{DelegateField, DelegateState};
use crate::tui::theme::THEME;

/// Widget for rendering the Delegate dialog
pub struct DelegateDialogWidget<'a> {
    /// Reference to dialog state
    state: &'a DelegateState,
    /// Dialog width as percentage (0-100)
    width_percent: u16,
}

impl<'a> DelegateDialogWidget<'a> {
    /// Create a new Delegate dialog widget
    pub fn new(state: &'a DelegateState) -> Self {
        Self {
            state,
            width_percent: 60,
        }
    ...
    pub fn render(&self, frame: &mut Frame) {
        ...
        let chunks = Layout::default()
            ...
            .constraints([
                ...
                Constraint::Length(1), // Available balance hint
        ...
        let instruction = Paragraph::new(Line::from(vec![Span::styled(
            "Enter Validator ID and Amount to delegate MON",
            THEME.metric_label(),
        )]))
        ...
        // Render each field
        self.render_field(frame, chunks[1], DelegateField::ValidatorId);
        self.render_field(frame, chunks[2], DelegateField::Amount);

        // Render available balance hint if set
        if let Some(balance) = self.state.available_balance {
            let hint = Line::from(vec![
                Span::styled("Available: ", THEME.label()),
                Span::styled(format!("{:.2} MON", balance), THEME.balance()),
            ]);
    ...
    fn render_field(&self, frame: &mut Frame, area: Rect, field: DelegateField) {
        ...
        let textarea = match field {
            DelegateField::ValidatorId => &self.state.validator_id,
            DelegateField::Amount => &self.state.amount,
        };
...
mod tests {
    ...
    #[test]
    fn test_delegate_dialog_widget_creation() {
        let state = DelegateState::new();
        let widget = DelegateDialogWidget::new(&state);
        assert_eq!(widget.width_percent, 60);
    }

    #[test]
    fn test_delegate_dialog_widget_with_width() {
        let state = DelegateState::new();
        let widget = DelegateDialogWidget::new(&state).with_width(80);
        assert_eq!(widget.width_percent, 80);
    }

    #[test]
    fn test_delegate_dialog_widget_calculate_dialog_area() {
        let state = DelegateState::new();
        let widget = DelegateDialogWidget::new(&state).with_width(60);
